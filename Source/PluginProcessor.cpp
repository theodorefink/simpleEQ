...
void SimpleEQAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    //do this by passing a processSpec object to the chains, which will then pass it to each link in the chains.
    
    juce::dsp::ProcessSpec spec;
    spec.maximumBlockSize = samplesPerBlock;
    spec.numChannels = 1; // one since mono chains
    ...
    *leftChain.get<ChainPositions::Peak>().coefficients = *peakCoefficients;
    ...
    auto cutCoefficients = juce::dsp::FilterDesign<float>::designIIRHighpassHighOrderButterworthMethod(chainSettings.lowCutFreq, sampleRate, 2*(chainSettings.lowCutSlope + 1)); //because of the way HighOrderButterworth whatver is implemented, it takes 2 4 6 and 8 for our 12, 24, 36, 48 filters since it divides by two and then applies that many 12 db/oct filters.
    
    //get lowcut chain and bypass all links (4 links in chain)
    ...
    //switch statement to get the coefficients and unbypass the number of filters depending on slope
...
void SimpleEQAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    //we get the settings or coefficients before the block is processed.
    auto chainSettings = getChainSettings(apvts);
    ...
    *rightChain.get<ChainPositions::Peak>().coefficients = *peakCoefficients;
    ...
    /*
     create audio block to wrap the buffer, use helper function from
     audio block class to extract individual channels
     from buffer which will then be wrapped inside more audio blocks
     */
    juce::dsp::AudioBlock<float> block(buffer);
    
    //audio blocks representing each channel
    auto leftBlock = block.getSingleChannelBlock(0);
    auto rightBlock = block.getSingleChannelBlock(1);
    
    //processing contexts to wrap these blocks that chain can use
...
//set up thing to extract the settings values from the parameters
